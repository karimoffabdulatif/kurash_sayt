...
const NAV = {
  ...
  about: [
    { uz: "Assotsiatsiya haqida", en: "About the Association", ru: "Об Ассоциации",   href: "/about"     },
    { uz: "Rasmiylar",            en: "Officials",             ru: "Официальные лица", href: "/directing" },
...
const T = {
  ...
  fed:     { uz: "Assotsiatsiya", en: "Association", ru: "Ассоциация" },
  ...
  brand1:  { uz: "Belbog'li Kurash Assotsiatsiyasi", en: "Belt Wrestling Association", ru: "Ассоциация борьбы на поясах" },
  brand2:  {
    uz: "Butunjahon Belbog'li Kurash va Bel Olish Kurash Assotsiatsiyasining rasmiy veb-sahifasi",
    en: "Official website of the World Belt Wrestling and 'Bel Olish' Wrestling Association.",
    ru: "Официальный веб-сайт Всемирной ассоциации борьбы на поясах и борьбы «бел олиш»",
  },
  brand3:  {
  ...
  copy2: {
    uz: "Butunjahon Belbog'li Kurash va Bel Olish Kurash Assotsiatsiyasi.",
    en: "World Belt Wrestling and Bel Olish Kurash Association.",
    ru: "Всемирная ассоциация борьбы на поясах и борьбы бел олиш.",
  },
  copied:  { uz: "Nusxalandi!",  en: "Copied!",  ru: "Скопировано!" },
  tapCall: { uz: "Qo'ng'iroq",   en: "Call",     ru: "Позвонить"    },
  tapMail: { uz: "Xat yozish",   en: "Send mail", ru: "Написать"    },
  tapMap:  { uz: "Xaritada ko'r", en: "View map", ru: "На карте"    },
};

const SOCIALS = [
  {
    name: "Telegram", href: "https://t.me/jaka_org", color: "hover:bg-[#229ED9]",
  ...
  {
    name: "Instagram", href: "https://instagram.com/jaka_org", color: "hover:bg-[#E1306C]",
  ...
  {
    name: "YouTube", href: "https://youtube.com/@jaka_org", color: "hover:bg-[#FF0000]",
  ...
  {
    name: "Facebook", href: "https://facebook.com/wbkboka", color: "hover:bg-[#1877F2]",
...
/* ─── ContactItem — tel, email, map uchun universal komponent ─────────── */
/* 
   - tel:   → window.location.href (eng ishonchli)
   - mailto → avval mailto ochadi, 400ms kutadi, agar hech narsa bo'lmasa
               Gmail webga o'tadi + emailni clipboard ga nusxalaydi
   - http   → window.open yangi tabda
*/
function ContactItem({ item, language, className }) {
  const [copied, setCopied]   = useState(false);
  const [showTip, setShowTip] = useState(false);

  const label = typeof item.label === "object" ? item.label[language] : item.label;
  const isEmail = item.href.startsWith("mailto:");
  const isTel   = item.href.startsWith("tel:");
  const isMap   = item.href.startsWith("http");

  const handleClick = (e) => {
    e.preventDefault();

    if (isTel) {
      /* Telefon — eng ishonchli usul */
    ...
    if (isEmail) {
      ...
      mailtoLink.href = item.href;
      mailtoLink.click();

      /* 3. Agar 600ms ichida hech narsa bo'lmasa — Gmail web ochish */
      const t = setTimeout(() => {
        setShowTip(true);
        setTimeout(() => setShowTip(false), 4000);
      }, 600);

      /* Sahifa focus yo'qotsa (mail app ochildi) — timeout bekor */
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {/* Copy indicator */}
        {isEmail && copied && (
          <span className="flex items-center gap-1 text-green-300 text-[10px] font-bold flex-shrink-0">
            <MdCheck className="w-3.5 h-3.5" />
            {T.copied[language]}
          </span>
        )}
        {isEmail && !copied && (
          <MdContentCopy className="w-3 h-3 opacity-40 flex-shrink-0" />
        )}
      </button>

      {/* Gmail fallback tooltip */}
      {showTip && (
        <div className="absolute left-0 right-0 top-full mt-1 z-50 bg-white text-[#0f2a5e] rounded-xl shadow-xl px-4 py-3 text-xs font-medium border border-blue-100">
          <p className="mb-2">Mail ilovasi topilmadi. Gmail orqali yuboring:</p>
          <button
            onClick={openGmail}
            className="w-full bg-[#0f2a5e] text-white rounded-lg py-2 font-bold text-xs hover:bg-blue-700 transition-colors"
          >
            Gmail da ochish →
          </button>
        </div>
...
export default function Footer() {
  ...
  const tgBtn   = darkMode
    ? "bg-blue-500 hover:bg-blue-400 text-white"
    : "bg-white text-blue-700 hover:bg-blue-50";

  /* Mobile contact item style */
  const mobileContactCls =
    "w-full flex items-center gap-3 bg-white/15 border border-white/20 rounded-xl px-4 py-3 text-[13px] text-white/80 hover:text-white hover:bg-white/25 transition-all active:scale-[0.98]";

  /* Desktop contact item style */
  const desktopContactCls =
    "w-full flex items-center gap-2.5 text-white/75 hover:text-white text-[13px] transition-colors group";
  ...
  return (
    <footer className={`${bg} text-white transition-colors duration-300`}>
      ...
      <div className={`sm:hidden ${bgBrand} border-b border-white/20 px-5 py-6`}>
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            ...
            <p className="text-blue-100 text-[10px] tracking-[0.2em] uppercase mt-0.5">
              {T.brand1[language]}
            </p>
          </div>
        </div>
        <p className="text-white/70 text-[12px] leading-relaxed mb-5">{T.brand3[language]}</p>
      ...
      <div className="sm:hidden px-5 py-6 grid grid-cols-2 gap-x-6 gap-y-7">
        {[
          { key: "pages", data: NAV.main },
          { key: "fed",   data: NAV.about },
        ].map(({ key, data }) => (
          <div key={key}>
            <h4 className="text-white font-black text-[10px] tracking-[0.22em] uppercase mb-3">
              {T[key][language]}
            </h4>
          ...
          {[
            {
              ...
              label: "+998 97 800 19 68",
              href: "tel:+998978001968",
            },
            {
              icon: <FaEnvelope className="w-3.5 h-3.5" />,
              label: "jaka-org@mail.ru",
              href: "mailto:jaka-org@mail.ru",
            },
            {
              icon: <FaMapMarkerAlt className="w-3.5 h-3.5" />,
              label: { uz: "Toshkent, O'zbekiston", en: "Tashkent, Uzbekistan", ru: "Ташкент, Узбекистан" },
          ...
          <div className="sm:col-span-2 lg:col-span-1">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-blue-100 text-[10px] tracking-widest uppercase">{T.brand1[language]}</p>
              </div>
            </div>
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {NAV.main.map((l) => (
                <li key={l.href}>
                  <Link href={l.href}
                    className="text-white/75 hover:text-white text-[13px] transition-colors hover:translate-x-1 inline-block">
                    {l[language]}
                  </Link>
                </li>
              ))}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-5">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-2 text-[11px] sm:text-[12px] text-white/60">
          <p className="text-center sm:text-left">
            <Link href="/admin" className="text-white/60 hover:text-white/60 cursor-default">
              &copy;
            </Link>
            {" "}{new Date().getFullYear()} {T.copy2[language]} {T.copy[language]}
          </p>
