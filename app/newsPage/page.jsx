...
function FeaturedCard({ item, darkMode, language }) {
  ...
  const contentBg = darkMode ? "bg-[#0d1f3c]"   : "bg-white";
  ...
  return (
    <div
      ...
      className={`grid lg:grid-cols-2 rounded-2xl overflow-hidden ${cardBg} border ${darkMode ? "border-blue-900" : "border-gray-100"} cursor-pointer transition-all duration-300`}
      style={{
        boxShadow: hov
    ...
    >
      ...
      <div className="relative overflow-hidden" style={{ minHeight: 440 }}>
        ...
        <span className="absolute bottom-4 left-4 bg-white/15 backdrop-blur-md border border-white/20 text-white text-[9px] font-medium tracking-widest uppercase px-3 py-1.5 rounded-sm">
          ⭐ {T.featured[language]}
        </span>
      </div>

      {/* Content */}
      <div className={`flex flex-col justify-between p-6 sm:p-8 ${contentBg}`}>
        <div>
          ...
          <div className={`h-px ${dividerC} mb-4`} />

          <p className={`${textC} text-sm leading-relaxed line-clamp-3`}>
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col flex-1 p-5">
        ...
        <div className={`flex items-center justify-between pt-3 border-t ${darkMode ? "border-blue-900" : "border-gray-100"} mt-auto`}>
  ...
  return (
    <section className={`${bg} min-h-screen transition-colors duration-300`}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14 lg:py-16">
        ...
        <div
          ref={headerRef}
          className="mb-10 mt-12 lg:mt-20 sm:mb-12"
          style={{
            opacity: iv ? 1 : 0,
